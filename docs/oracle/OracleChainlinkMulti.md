...
*This contract concerns an oracle that uses Chainlink with multiple pools to read fromIt inherits the `ModuleChainlinkMulti` contract and like all oracle contracts, this contract is an instance of `OracleAstract` that contains some base functions*
...
Reads the rate from the Chainlink circuit
...
Converts an in-currency quote amount to out-currency using Chainlink&#39;s circuit
