...
describe('UniswapV3Swapper', () => {
    ...
    it('should get input amount', async () => {
        ...
        const amountIn = await uniswapV3Swapper.getInputAmount(swapData, '0x00');

        expect(amountIn.gt(0)).to.be.true;
    })

    it('should swap', async () => {
        const { uniswapV3Swapper, deployer, binanceWallet, weth, usdc, createSimpleSwapData } = await loadFixture(registerFork);
        const amount = BN(1e18);
    ...
    it('should swap assets available in YB', async () => {
        const { uniswapV3Swapper, yieldBox, deployer, binanceWallet, weth, usdc, wethAssetId, usdcAssetId, createYbSwapData } = await loadFixture(registerFork);
        ...
        await yieldBox.transfer(deployer.address, uniswapV3Swapper.address, wethAssetId, share);
        await uniswapV3Swapper.swap(swapData, 0, deployer.address, '0x');
