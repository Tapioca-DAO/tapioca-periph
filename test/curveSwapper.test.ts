...
describe('CurveSwapper', () => {
    it('should get output amount', async () => {
        ...
        const swapData = createSimpleSwapData(usdt.address, usdc.address, amount, 0);
    ...
    it('should get input amount', async () => {
        ...
        await expect(curveSwapper.getInputAmount(
            swapData,
            '0x',
        )).to.be.reverted;
    });
    ...
    it('should swap', async () => {
        const { curveSwapper, usdt, usdc, binanceWallet, deployer, createSimpleSwapData } = await loadFixture(registerFork);
    ...
    it('should swap assets available in YB', async () => {
        const { curveSwapper, usdt, usdc, usdtAssetId, usdcAssetId, binanceWallet, deployer, yieldBox, createYbSwapData } = await loadFixture(registerFork);
        ...
        await yieldBox.depositAsset(usdtAssetId, deployer.address, deployer.address, 0, share);


        const ybUsdtBalanceBefore = await yieldBox.balanceOf(deployer.address, usdtAssetId);
        ...
        await yieldBox.transfer(deployer.address, curveSwapper.address, usdtAssetId, share);
        await curveSwapper.swap(swapData, 1, deployer.address, data);

        const ybUsdtBalanceAfter = await yieldBox.balanceOf(deployer.address, usdtAssetId);
