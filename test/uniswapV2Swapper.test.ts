...
describe('UniswapV2Swapper', () => {
    it('should get output amount', async () => {
        ...
        const swapData = createSimpleSwapData(weth.address, usdc.address, amount, 0);
    ...
    it('should get input amount', async () => {
        ...
        const swapData = createSimpleSwapData(weth.address, usdc.address, 0, amount);
        const amountIn = await uniswapV2Swapper.getInputAmount(swapData, '0x00');

        expect(amountIn.gt(0)).to.be.true;
    })

    it('should swap', async () => {
        const { uniswapV2Swapper, deployer, binanceWallet, weth, usdc, createSimpleSwapData } = await loadFixture(registerFork);
        const amount = BN(1e18);
    ...
    it('should swap assets available in YB', async () => {
        const { uniswapV2Swapper, yieldBox, deployer, binanceWallet, weth, usdc, wethAssetId, usdcAssetId, createYbSwapData } = await loadFixture(registerFork);
        ...
        await yieldBox.depositAsset(wethAssetId, deployer.address, deployer.address, 0, share);

        const ybWethBalanceBefore = await yieldBox.balanceOf(deployer.address, wethAssetId);
        expect(ybWethBalanceBefore.eq(share)).to.be.true;

        const ybUsdcBalanceBefore = await yieldBox.balanceOf(deployer.address, usdcAssetId);
        expect(ybUsdcBalanceBefore.eq(0)).to.be.true;

        const swapData = createYbSwapData(wethAssetId, usdcAssetId, share, 0);
        await yieldBox.transfer(deployer.address, uniswapV2Swapper.address, wethAssetId, share);
        await uniswapV2Swapper.swap(swapData, 0, deployer.address, '0x');

        const ybWethBalanceAfter = await yieldBox.balanceOf(deployer.address, wethAssetId);
        expect(ybWethBalanceAfter.eq(0)).to.be.true;

        const ybUsdcBalanceAfter = await yieldBox.balanceOf(deployer.address, usdcAssetId);
