...
// Tapioca
import {DepositAndSendForLockingData, LockAndParticipateData} from "tapioca-periph/interfaces/periph/IMagnetar.sol";
...
contract MagnetarMintExternalHelper {
    function mintBBLendXChainSGLEncoder(bytes memory composeMsg, uint256 amountToSet)
        public
        pure
        returns (bytes memory)
    {
        ...
        return TapiocaOmnichainEngineCodec.encodeToeComposeMsg(abi.encode(lendData), msgType_, msgIndex_, nextMsg_);
    }

    function depositYBLendSGLLockXchainTOLPEncoder(bytes memory composeMsg, uint256 amountToSet)
        public
        pure
        returns (bytes memory)
    {
        ...
        return TapiocaOmnichainEngineCodec.encodeToeComposeMsg(abi.encode(lockData), msgType_, msgIndex_, nextMsg_);
    }
}
