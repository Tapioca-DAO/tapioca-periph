...
abstract contract ComposableOFTCore is OFTCore, IComposableOFTCore {
    ...
    function estimateSendAndCallFee(
        ...
        bytes memory payload =
            abi.encode(PT_SEND_AND_CALL, abi.encodePacked(msg.sender), _toAddress, _amount, _payload, _dstGasForCall);
    ...
    function _sendAndCall(
        ...
        bytes memory lzPayload =
            abi.encode(PT_SEND_AND_CALL, abi.encodePacked(msg.sender), _toAddress, amount, _payload, _dstGasForCall);
    ...
    function _sendAndCallAck(uint16 _srcChainId, bytes memory _srcAddress, uint64 _nonce, bytes memory _payload)
        ...
        virtual
    {
        (, bytes memory from, bytes memory toAddress, uint256 amount, bytes memory payload, uint64 gasForCall) =
            abi.decode(_payload, (uint16, bytes, bytes, uint256, bytes, uint64));
    ...
    function _safeCallOnOFTReceived(
        ...
        (bool success, bytes memory reason) = _to.excessivelySafeCall(
            ...
            abi.encodeWithSelector(
                IOFTReceiver.onOFTReceived.selector, _srcChainId, _srcAddress, _nonce, _from, _amount, _payload
            )
        );
        if (!success) {
            failedOFTReceivedMessages[_srcChainId][_srcAddress][_nonce] =
                keccak256(abi.encode(_from, _to, _amount, _payload));
